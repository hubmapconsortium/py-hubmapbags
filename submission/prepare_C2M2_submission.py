...
# Lastmod date unless I forgot to change it: 2024-08-28
# 
##########################################################################################
...
idMap = {
    ...
    'entrez_to_ensembl': '%s/HPO_MPO_Entrez_gene_IDs_to_EnsEMBL_IDs.tsv' % cvRefDir,
    'hpo_to_entrez': '%s/hp.phenotype_to_genes.txt' % cvRefDir,
    'mpo_to_entrez': '%s/mp.phenotype_to_genes.txt' % cvRefDir
}
...
def decorateTermsUsed(  ):
    ...
    for psource in ('hpo', 'mpo'):
             
         mapFile = idMap[psource + '_to_entrez']
         ...
         with open( mapFile, 'r' ) as IN:
              ...
              for line in IN:
                   ...
                   if not ( phen_id in hpo_to_entrez ):
                
                        hpo_to_entrez[phen_id] = {}

                   hpo_to_entrez[phen_id][entrez_id] = 1

              # end ( line iterator on the hpo-to-entrez map )

         # end with ( open mapFile as IN )

    # end for ( psource in ('hpo', 'mpo') )
