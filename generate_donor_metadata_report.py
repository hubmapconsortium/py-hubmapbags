...
today = date.today()
filename = 'donor-metadata-report-' + str(today).replace('-','') + '.tsv'
...
def has_donor_metadata( hubmap_id, instance='prod', token=None ):
	donor = hubmapbags.apis.get_donor_info( hubmap_id, instance=instance, token=token, debug=True )	
	if hasattr(donor,'metadata'):
		return True
	else:
		return False
...
table = []
headers = ['Status', 'Assay type', 'HuBMAP ID', 'Group name', 'Has Donor Metadata']
for assay in assays:
	print(assay)
		...
		if dataset is not None:
			if id['data_type'] == assay and id['status'] == 'Published':
				datum = [id['status'], \
					...
					id['hubmap_id'], \
					id['group_name'], \
					has_donor_metadata( hubmap_id, instance='prod', token=token )]
				table.append(datum)
